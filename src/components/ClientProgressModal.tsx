    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Resumo do Progresso */}
          <Card className="p-4 border-l-4" style={{ borderLeftColor: getStatusColor().replace('bg-', '#').replace('-500', '500') }}>
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-lg font-semibold">Meta: {goal} posts</h3>
              <span className={cn("text-sm font-bold px-3 py-1 rounded-full text-white", getStatusColor())}>
                {progressStatus}
              </span>
            </div>
            ...
            <Progress value={percentage} className="mt-3 h-2" indicatorClassName={getStatusColor()} />
          </Card>
