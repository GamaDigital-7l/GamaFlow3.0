...
const ClientNotesSection: React.FC<ClientNotesSectionProps> = ({ clientId, clientName }) => {
  ...
  // Filtra as notas para mostrar apenas as do cliente atual
  ...
  const handleNewNote = () => {
    if (!user) {
        showError("Você precisa estar logado para criar anotações.");
        return;
    }
    const tempNote: Note = {
      id: 'new',
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      clientId: clientId, // Vincula ao cliente atual
    };
  ...
  const handleDuplicateNote = (note: Note) => {
    if (!user) {
        showError("Você precisa estar logado para duplicar anotações.");
        return;
    }
  ...
  const filteredClientNotes = useMemo(() => {
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">Carregando anotações do cliente...</p>
      </div>
    );
  }
      ...
      <div className="w-full sm:w-60 flex-shrink-0 border-r bg-muted/30 flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-2 text-sm">
            ...
            <div className="space-y-1">
                <div className="p-2 rounded-lg hover:bg-muted/50 cursor-pointer">Todas as Notas ({clientNotes.length})</div>
      ...
      <div className="flex-grow flex flex-col bg-background">
        {selectedNote ? (
          ...
          <div className="flex items-center justify-center h-full text-muted-foreground">
            Crie uma nova anotação para este cliente.
          </div>
        )}
      </div>
    </div>
