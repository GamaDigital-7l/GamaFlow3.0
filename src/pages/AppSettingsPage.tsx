...
import { useAiSummary } from '@/hooks/use-ai-summary'; // Importando useAiSummary

const AppSettingsPage: React.FC = () => {
  const { settings, updateSettings, isSaving, isLoading } = useAppSettings();
  const { testAiSummary, isLoading: isAiLoading } = useAiSummary(); // Usando o hook de AI
  ...
  // Simulação de upload de arquivo (apenas para fins de URL temporário)
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'light' | 'dark') => {
    const file = e.target.files?.[0];
    if (file) {
      // Em um ambiente real, faríamos o upload para o Supabase Storage aqui.
      ...
      showSuccess('Arquivo selecionado para preview. Clique em Salvar para persistir.');
    }
  };
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="grid gap-2">
                        ...
                        <Select value={aiProvider} onValueChange={(value) => setAiProvider(value as 'groq' | 'openai')} disabled={isSaving}>
                            <SelectTrigger>
                ...
                <Button 
                    ...
                    disabled={isSaving || isTesting || !aiEnabled || !aiApiKey.trim()}
                >
