...
// Componente para a lista de notas (coluna central)
...
const NoteList: React.FC<NoteListProps> = ({ notes, selectedNote, onSelectNote, onPinToggle, onDelete, isMutating }) => {
    
    // Função para extrair um snippet de texto simples do HTML
    ...
    return (
        <div className="w-full sm:w-80 flex-shrink-0 border-r bg-muted/30 flex flex-col">
...
const NotesPage: React.FC = () => {
  ...
  const [noteContent, setNoteContent] = useState(''); // Conteúdo HTML
  const [noteTags, setNoteTags] = useState<string[]>([]); // Novo estado para tags
  ...
  // Debounce para tags (salva imediatamente após adicionar/remover)
  const debouncedTags = useDebounce(noteTags, 1000); 

  // Efeito para sincronizar o estado local com a nota selecionada
  useEffect(() => {
    if (selectedNote) {
  ...
  // Efeito para salvar automaticamente (Título, Conteúdo e Tags)
  useEffect(() => {
    // Verifica se houve mudança no título, conteúdo OU tags
    ...
    if (selectedNote && selectedNote.id !== 'new' && (hasTitleChanged || hasContentChanged || hasTagsChanged)) {
      ...
      if (debouncedTitle.trim() || debouncedContent.trim()) {
        ...
        // O hook upsertNote lida com a atualização do updatedAt no DB
        upsertNote(updatedNote, {
            onSettled: () => setIsSavingIndicator(false),
            onSuccess: (savedNote) => {
                setSelectedNote(savedNote); // Atualiza o selectedNote com a versão salva para evitar loop de debounce
            }
        });
      }
    }
  ...
  const handleNewNote = () => {
    if (!user) return;
    // Cria uma nota temporária para iniciar a edição
    const tempNote: Note = {
      id: 'new',
      userId: user.id, 
      title: 'Nova Anotação',
  ...
  const filteredNotes = useMemo(() => {
    ...
    return notes.filter(note => 
      ...
      // Não busca no conteúdo HTML, apenas no título e tags para performance
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">Carregando anotações...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-[85vh] border rounded-lg overflow-hidden shadow-xl">
      ...
      {/* Coluna 1: Sidebar de Navegação (Pastas/Tags) - Simulação */}
      <div className="w-full sm:w-60 flex-shrink-0 border-r bg-muted/30 flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-2 text-sm">
            ...
            <div className="space-y-1">
                <div className="p-2 rounded-lg hover:bg-muted/50 cursor-pointer">#Clientes</div>
