...
const PostApprovalPage: React.FC = () => {
  ...
  const { getClientById, getPostById, updatePost } = useClientStore();
  ...
  useEffect(() => {
    if (clientId && postId) {
      const client = getClientById(clientId);
      const fetchedPost = getPostById(clientId, postId);
    ...
    setIsLoading(false);
  }, [clientId, postId, getClientById, getPostById]);
          ...
          {isPendingApproval && !isLinkExpired ? (
            <div className="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-4 pt-4">
              ...
              <Button 
                ...
                className="border-gray-300 text-gray-700 hover:bg-gray-100 px-6 w-full sm:w-auto" // Estilo neutro
                disabled={isSubmitting}
              >
