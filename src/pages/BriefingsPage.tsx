    ...
    <div className="space-y-6">
          ...
          forms.map(form => (
            <Card key={form.id} className="flex flex-col justify-between h-full">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                    <span>Campos: <span className="font-medium text-foreground">{form.fields.length}</span></span>
                    <Button 
                        variant="link" 
                        size="sm" 
                        onClick={() => handleOpenResponses(form)}
                        className="p-0 h-auto text-dyad-500 hover:text-dyad-600"
                    >
                        Respostas: <span className="font-bold text-foreground ml-1">{responseCounts[form.id] || 0}</span>
                    </Button>
                </div>
      ...
      <Dialog open={isResponsesDialogOpen} onOpenChange={handleCloseResponsesDialog}>
        <DialogContent className="sm:max-w-[900px] max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Visualizar Respostas</DialogTitle>
          </DialogHeader>
          {viewingForm && (
